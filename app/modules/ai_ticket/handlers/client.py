...
async def handle_ai_ticket_message(
    ...
    try:
        result = await ForumService.get_or_create_ticket(db, bot, db_user.id, db_user.full_name)
        
        # Проверяем результат - это может быть тикет или tuple (None, 'fallback')
        if isinstance(result, tuple) and result[0] is None:
            reason = result[1]
            if reason == 'fallback':
                # Фоллбек на стандартную систему тикетов
                logger.warning('ai_ticket_client.fallback_to_standard_tickets', user_id=db_user.id)
                from app.handlers.tickets import _create_standard_ticket_fallback
                await _create_standard_ticket_fallback(message, db, db_user, user_text)
                return
            else:
                # Критическая ошибка
                logger.error('ai_ticket_client.ticket_init_critical_error', user_id=db_user.id)
                await message.answer(texts.t('TICKET_CREATE_ERROR', '⚠️ Не удалось создать обращение. Пожалуйста, попробуйте позже.'))
                return
