...
class ForumService:
    ...
    async def get_or_create_ticket(
    ...
    ) -> ForumTicket | tuple[None, str]:
        """
        Get an existing open ticket for the user, or create a new one
        (including a Telegram Forum topic in the manager group).
        
        Returns:
            ForumTicket - успешно создан/найден тикет
            (None, 'fallback') - ошибка Forum, нужен фоллбек на стандартные тикеты
            (None, 'error') - критическая ошибка
        """
        ...
        if not forum_group_id:
            ...
            await ForumService._notify_admins_forum_error(
                bot, 
                '⚠️ <b>AI-Ticket: Forum ID не настроен!</b>\n\n'
                'Пользователь пытался создать тикет, но SUPPORT_AI_FORUM_ID не указан.\n'
                'Тикет будет создан через стандартную систему.\n\n'
                '➡️ Настройте ID Forum-группы в админке:\n'
                '<i>Поддержка → Настройки → ID Forum-группы</i>'
            )
        ...
        try:
            ...
            error_str = str(e)
            logger.error('forum_service.create_topic_failed', error=error_str, user_id=user_id, forum_id=forum_group_id)
            
            # Определяем тип ошибки и формируем понятное сообщение для админа
            if 'chat not found' in error_str.lower():
                error_reason = (
                    '❌ <b>Группа не найдена</b>\n'
                    f'ID: <code>{forum_group_id}</code>\n\n'
                    'Возможные причины:\n'
                    '• Неверный ID группы\n'
                    '• Бот не добавлен в группу\n'
                    '• Группа была удалена'
                )
            elif 'not enough rights' in error_str.lower() or 'administrator' in error_str.lower():
                error_reason = (
                    '❌ <b>Недостаточно прав</b>\n'
                    f'ID группы: <code>{forum_group_id}</code>\n\n'
                ...
                error_reason = (
                    ...
                    f'Указанный ID: <code>{forum_group_id}</code>\n\n'
                    'Проверьте правильность ID Forum-группы.\n'
                    'ID должен начинаться с -100'
                )
            elif 'forum' in error_str.lower() and 'not' in error_str.lower():
                error_reason = (
                    '❌ <b>Группа не является Forum</b>\n'
                    f'ID: <code>{forum_group_id}</code>\n\n'
                    'Включите темы (Topics) в настройках группы:\n'
                    '<i>Группа → Настройки → Темы</i>'
                )
            else:
                error_reason = (
            ...
            await ForumService._notify_admins_forum_error(
                bot,
                f'⚠️ <b>AI-Ticket: Ошибка создания топика!</b>\n\n'
                f'{error_reason}\n\n'
                f'👤 Пользователь: {user_display_name} (ID: {user_id})\n'
                f'Тикет будет создан через стандартную систему.'
            )
            return (None, 'fallback')
    ...
    @staticmethod
    async def _notify_admins_forum_error(bot: Bot, message: str) -> None:
        """Отправляет уведомление админам о проблеме с Forum."""
        try:
            from app.config import settings
            ...
            for admin_id in admin_ids[:3]:  # Первые 3 админа
                try:
                    ...
                    logger.warning('forum_service.admin_notify_failed', admin_id=admin_id, error=str(e))
        except Exception as e:
            logger.error('forum_service.admin_notify_error', error=str(e))
